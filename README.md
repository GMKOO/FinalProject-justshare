...
### 🧑‍🤝‍🧑 맴버구성
 ...
 - 팀원 :  대규 - 메인페이지 및 로그인, API를 이용한 통계 , 차트 그리기 , 관리자 페이지  
 - 팀원 :  재윤 - 회원가입 및 메인페이지 , API 기반 위치(GPS) 정보 활성화,GABIA 배포 및 GIT 관리
 - 팀원 :  상민 - 마이 페이지 
 - 팀원 :  종휘 - 게시물 작성 양식 및 파일 업로드 , 검색 필터, 찜, 신고, 리뷰,A PI를 이용한 지도 그리기
...
- 사용자와 첫 대화시 신규 메시지 알람 팝업 - 클릭시 대화 목록 리스트로 이동
- 거래요청 알람 팝업 -클릭시 거래화면으로 이동
- 거래완료 알람 팝업 -클릭시 리뷰작성으로 이동
- 대화목록리스트에서 사용자별 읽지 않은 신규 메시지 count 기능
- footer에 모든 사용자에 읽지 않은 신규 메시지 합산 count 기능
